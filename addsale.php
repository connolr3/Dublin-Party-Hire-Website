...
<!-- PhP Code that will determine the input and figure out error messages -->
    ...
    return $data;
        ...
        $category = $product_id = $sale = $currsale = $product_name = "";
        ...
        if(empty($_POST['product_id']))
        {
            ...
            $q3 = "SELECT sale, product_name FROM equipment WHERE id = '$product_id'";
            $result3 = $db->query($q3);
        ...
        //make sure sale is valid
        if(!is_numeric($_POST['sale']))
        {
            $priceErr = "Please enter a valid sale value";
        }else{
            $sale = test_input($_POST['sale']);
        }


        //make sure sale if in correct format i.e a decimal
        if($_POST['sale']>1)
        {
            $priceErr = "Please enter sale in decimal form";
        }else{
            $sale = test_input($_POST['sale']);
        }
    ...
    $sql  = "UPDATE equipment SET sale = '$sale' WHERE id = '$product_id'";

    $result_sql = $db->query($sql);

    $update_message = "Thank you, the sale price has been updated.";
    $update_message2 = "The new sale on ".$product_name." is ".$sale." (".($sale*100)."% off)";
   ...
   //clear form
    $category = $product_id = $rental_price = $currsale = $product_name = "";
...
<div class="w3-container w3-content w3-padding-11" style="max-width:800px">
    ...
    <h2>Use this form to change the sale for a specific product:</h2><br>

    <?PHP
        echo "<b>Current Sales;</b><br>";
        include ("detail.php"); 
        session_start();
...
<tr>
<td>
<h5><b>New Sale (Please enter in decimal form)</b></h5>
<p >
      <?php 
      echo "To remove a sale enter 0.<br>";
            echo "Current Sale: ".$currsale." (".($currsale*100)."% off)";
        ?>
      </p>
<input class= "w3-input w3-border" type="number" name = "sale" max="100000.00" min="0.00" step="0.01" >
...
{
  var x = confirm("Are you sure you want to update the sale price?");
  if (x)
      return true;
  else
    return false;
...
<button class="w3-button w3-black" onclick="return ConfirmDelete();" type="submit" value="Update Price" name="actionupdate" >Update Sale</button>
