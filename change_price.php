<!-- This is the change equipment rental price page that lets the admin change the rental price for each equipment
...
<!-- PhP Code that will determine the input and figure out error messages -->
    ...
    return $data;
        ...
        if(empty($_POST['product_id']))
        {
            ...
            $q3 = "SELECT rental_price_excl_vat, product_name FROM equipment WHERE id = '$product_id'";
            $result3 = $db->query($q3);
            $row = mysqli_fetch_assoc($result3);

            //get current rental price
            $currprice = $row['rental_price_excl_vat'];
        ...
        if(!is_numeric($_POST['rental_price']))
        {
...
if($priceErr == "" && $categoryErr == "" && $productErr == "" ){
    ...
    $sql  = "UPDATE equipment SET rental_price_excl_vat = '$rental_price' WHERE id = '$product_id'";

    $result_sql = $db->query($sql);

    $update_message = "Thank you, the rental price has been updated.";
    $update_message2 = "The new rental price for ".$product_name." is: €".$rental_price;
...
<h2>Use this form to change the rental price for a specific product:</h2>
...
<input class= "w3-input w3-border" type="number" name = "rental_price" max="100000.00" min="0.01" step="0.01" >
...
{
  var x = confirm("Are you sure you want to update the rental price?");
  if (x)
      return true;
  else
    return false;
